...
| **EDA**      | `synth_profile`                   | Choice (str) | {`balanced_default`, `timing_aggressive`, `power_aggressive`, `area_aggressive`, `exact_map`} | DC 合成策略：balanced（標準）、timing（時序優先）、power（功耗優先）、area（面積優先，可能違反 timing）、exact_map（保留階層） |
...
本框架的搜尋空間由 `conf/params_prop/cimloop.yaml` 定義，可透過 `run_exploration.py` 執行 DSE。輸出結果（`dse_results.json`）除最終目標（accuracy, energy_uj, timing_us, area_mm2, hv）外，亦包含 Path 2 原始指標（p2_area_um2, p2_timing_slack_ns, p2_dynamic_power_mw）與 Path 3 指標（p3_execution_cycles, p3_dynamic_power_mw），供後續分析與圖表繪製。

為驗證本框架在不同面向的貢獻，我們設計了以下四大實驗情境 (Scenarios)：
