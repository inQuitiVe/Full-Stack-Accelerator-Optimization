...
Reads a JSON parameter dictionary (from stdin or function call) and:
  ...
  3. Patches `../fsl-hd/tcl/synth_dse.tcl` (from dc/synth_template.tcl) with the
     correct `create_clock` period derived from the `frequency` parameter.
  4. Injects the synthesis strategy TCL block based on `synth_profile`.
  5. Writes `../fsl-hd/tb/tb_macros.svh` with Testbench constants for Path 3.
...
Server deployment layout assumed:
  ~/workspace/
    ...
    │   └── tb/                          ← tb_macros.svh written here
    └── full-stack-opt/                  ← this script lives here
        └── dc/synth_template.tcl        ← TCL template
...
TCL_TEMPLATE     = SCRIPT_DIR / "dc" / "synth_template.tcl"
TCL_OUTPUT       = FSL_HD_DIR / "tcl" / "synth_dse.tcl"
TB_MACROS_OUTPUT = FSL_HD_DIR / "tb" / "tb_macros.svh"
...
def _inject_synth_profile(profile: str, template_path: Path, output_path: Path) -> None:
    ...
    if profile not in _SYNTH_PROFILE_TCL:
        raise ValueError(
            f"Unknown synth_profile {profile!r}. "
...
def _write_tb_macros(params: dict, output_path: Path) -> None:
    """
    Generate `hardware/vcs/tb_macros.svh` with Testbench constants derived
    from the current BO parameter configuration.

    Generated macros:
      TB_CLK_PERIOD_NS  — clock half-period for the testbench clock generator
      TB_NUM_VECTORS    — number of test vectors to drive through the DUT
      TB_INPUT_FILE     — path to the dumped input hex file
      TB_GOLDEN_FILE    — path to the golden label hex file
      TB_WEIGHT_FILE    — path to the weight hex file
    """
    frequency_hz: float = float(params.get("frequency", 1e8))
    period_ns: float = round(1e9 / frequency_hz, 4)
    num_vectors: int = int(params.get("num_tests", 50))

    # Hex files are placed in fsl-hd/tb/data/ by eda_server.py.
    # simv is invoked from fsl-hd/, so paths below are relative to fsl-hd/.
    lines = [
        "// AUTO-GENERATED by json_to_svh.py — DO NOT EDIT MANUALLY",
        "`ifndef TB_MACROS_SVH",
        "`define TB_MACROS_SVH",
        "",
        f"`define TB_CLK_PERIOD_NS  {period_ns}",
        f"`define TB_NUM_VECTORS    {num_vectors}",
        '`define TB_INPUT_FILE     "tb/data/inputs.hex"',
        '`define TB_GOLDEN_FILE    "tb/data/labels.hex"',
        '`define TB_WEIGHT_FILE    "tb/data/weights.hex"',
        "",
        "`endif // TB_MACROS_SVH",
        "",
    ]
...
def translate(params: dict) -> None:
    ...
    Steps:
      ...
      2. Patch clock period into synth.tcl
      3. Inject synthesis strategy block into synth.tcl (based on synth_profile)
      4. Write testbench constants → tb_macros.svh (for Path 3 GLS)
    """
    ...
    synth_profile: str = params.get("synth_profile", _DEFAULT_SYNTH_PROFILE)
