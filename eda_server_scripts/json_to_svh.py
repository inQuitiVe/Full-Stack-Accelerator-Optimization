...
Reads a JSON parameter dictionary (from stdin or function call) and:
  ...
  3. Patches `hardware/dc/synth.tcl` with the correct `create_clock` period
     derived from the `frequency` parameter.
  4. Injects the synthesis strategy TCL block based on `synth_profile`.
  5. Writes `hardware/vcs/tb_macros.svh` with Testbench constants for Path 3.
...
TB_MACROS_OUTPUT = HARDWARE_DIR / "vcs" / "tb_macros.svh"
...
def _write_tb_macros(params: dict, output_path: Path) -> None:
    ...
    lines = [
        ...
        '`define TB_INPUT_FILE     "data/inputs.hex"',
        '`define TB_GOLDEN_FILE    "data/labels.hex"',
        '`define TB_WEIGHT_FILE    "data/weights.hex"',
        "",
        "`endif // TB_MACROS_SVH",
        "",
    ]
